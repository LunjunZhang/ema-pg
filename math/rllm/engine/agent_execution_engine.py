...
class AgentExecutionEngine:
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        if mode == "Text":
            ...
            prompt_tokens, response_tokens, response_masks, rollout_log_probs, is_valid_trajectory = self.assemble_steps(episode_steps)
            token_result = {
    ...
    def assemble_steps(self, steps: list[dict]):
        ...
        - steps: List of {"prompt": str, "response": str, "prompt_ids": list, "completion_ids": list, "logprobs": list}
        ...
        - rollout_log_probs: Log probabilities for response tokens (0 for env tokens, actual logprobs for completions)
        """
        ...
        for i, step in enumerate(steps):
            ...
            if i == 0:
                ...
                rollout_log_probs.extend(current_logprobs)
                ...
                if current_prompt_ids[: len(accumulated_sequence)] != accumulated_sequence:
                    ...
                    diff_pos = None
                    for j, (expected, actual) in enumerate(zip(accumulated_sequence, prefix, strict=False)):
                        if expected != actual:
                            diff_pos = j
                            break
                ...
                num_env_tokens = len(current_prompt_ids) - len(accumulated_sequence)
                response_tokens.extend(current_prompt_ids[len(accumulated_sequence) :] + current_completion_ids)
                response_masks.extend([0] * num_env_tokens + [1] * len(current_completion_ids))  # completion contributes to loss
                # Env tokens don't have log probs (not generated by model), use 0.0; completion tokens have actual log probs
                rollout_log_probs.extend([0.0] * num_env_tokens + list(current_logprobs))
                accumulated_sequence = current_prompt_ids + current_completion_ids

        assert len(response_masks) == len(response_tokens)
        assert len(rollout_log_probs) == len(response_tokens), f"Log probs length {len(rollout_log_probs)} != response tokens length {len(response_tokens)}"
        ...
        return prompt_tokens, response_tokens, response_masks, rollout_log_probs, is_valid_trajectory
