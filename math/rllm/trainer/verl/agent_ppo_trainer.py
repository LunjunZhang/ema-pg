...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def fit_agent(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            # Check if we should save validation results (for benchmark evaluation)
            save_val_results = self.config.get("benchmark", {}).get("save_val_results", False)
            val_output_path = self.config.get("benchmark", {}).get("output_path", None)

            if save_val_results and val_output_path:
                # _validate_agent handles max_response_length override and calls the right inner function
                val_metrics = self._validate_agent(save_output_path=val_output_path)
                pprint(f"Initial validation metrics (results saved to {val_output_path}): {val_metrics}")
            else:
            ...
            eval_ref_policy_freq = self.config.trainer.get("eval_ref_policy_freq", 0)
            if eval_ref_policy_freq > 0 and self.use_reference_policy and not self.ref_in_actor:
                ref_val_metrics = self._validate_ref_policy_agent()
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw):
                        ...
                        # Recompute old_log_probs using PyTorch forward pass.
                        #
                        # Why we can't use rollout_log_probs directly (bypass mode):
                        # We investigated using vLLM's rollout_log_probs to skip this forward pass, but discovered
                        # that vLLM V1 returns log probabilities computed on RAW logits (temperature=1.0), before
                        # any sampling parameter adjustments like temperature scaling. See:
                        # https://docs.vllm.ai/en/v0.8.1/getting_started/v1_user_guide.html
                        #
                        # From vLLM docs: "Logprobs in V1 are now returned immediately once computed from the
                        # model's raw output (i.e., before applying any logits post-processing such as temperature
                        # scaling or penalty adjustments)."
                        #
                        # For PPO, we need log probs computed WITH temperature scaling (e.g., temperature=0.6):
                        #   - vLLM returns: log_softmax(logits)           [no temperature]
                        #   - PPO needs:   log_softmax(logits / 0.6)      [with temperature]
                        #
                        # These differ significantly (recomputed values are more negative due to sharper distribution),
                        # with max differences of ~20 nats observed in testing. Using raw log probs would produce
                        # incorrect importance sampling ratios for PPO updates.
                        #
                        # Future: vLLM docs indicate "support for logprobs with post-sampling adjustments is in
                        # progress". Once available, we can revisit bypass mode to save this forward pass.
                        # Compute old_log_probs, dispatching based on KL loss type
                        kl_loss_type = self.config.actor_rollout_ref.actor.kl_loss_type
                        kl_topk_k = getattr(self.config.actor_rollout_ref.actor, "kl_topk_tokens", None)

                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            if kl_loss_type == "full_reverse":
                                # Reverse KL: old policy computes the top-k indices (or full logits)
                            ...
                            if "rollout_log_probs" in batch.batch.keys():
                                # Track difference between rollout and recomputed log probs for monitoring.
                                # Due to vLLM V1's raw log prob behavior (see comment above), these will differ.
                        ...
                        if self.use_reference_policy:
                            # compute reference log_prob (and optionally logits for full KL)
                                ...
                                if kl_loss_type == "full_reverse":
                                    if kl_topk_k is not None and kl_topk_k > 0:
                                        ...
                                        if not self.ref_in_actor:
                                            ref_policy_output = self.ref_policy_wg.compute_ref_log_prob_at_indices(batch)
                                        else:
                                            ref_policy_output = self.actor_rollout_wg.compute_ref_log_prob_at_indices(batch)
                                    else:
                                        # Reverse KL without top-k: get full ref logits
                                        batch.meta_info["kl_topk_k"] = -1
                                        if not self.ref_in_actor:
                                            ref_policy_output = self.ref_policy_wg.compute_ref_log_prob_with_logits(batch)
                                        else:
                                            ref_policy_output = self.actor_rollout_wg.compute_ref_log_prob_with_logits(batch)
                                elif kl_loss_type == "full_forward":
                                    # Forward KL: use unified function with appropriate kl_topk_k
                                    effective_topk = kl_topk_k if (kl_topk_k is not None and kl_topk_k > 0) else -1
                                    batch.meta_info["kl_topk_k"] = effective_topk
                                    if not self.ref_in_actor:
                                        ref_policy_output = self.ref_policy_wg.compute_ref_log_prob_with_logits(batch)
                                    ...
                                    if not self.ref_in_actor:
                                        ref_policy_output = self.ref_policy_wg.compute_ref_log_prob(batch)
                                    else:
                                        ref_policy_output = self.actor_rollout_wg.compute_ref_log_prob(batch)
                                batch = batch.union(ref_policy_output)

                        # compute rewards with KL penalty if needed
                        # Note: use_kl_in_reward applies KL penalty to rewards (non-differentiable, affects GAE)
                        # This is separate from actor.use_kl_loss which applies KL in the policy loss (differentiable)
                        if self.config.algorithm.use_kl_in_reward:
                            batch, kl_metrics = apply_kl_penalty(
                                batch,
                                kl_ctrl=self.kl_ctrl_in_reward,
                                kl_penalty=self.config.algorithm.kl_penalty,
                                kl_penalty_at_final_token=self.config.algorithm.kl_penalty_at_final_token,
                            )
                    ...
                    # Compute data metrics before masking truncated samples
                    # This preserves the original behavior where metrics reflect the full batch
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        #   - use_reference_policy=True (KL penalty enabled)
                        #   - ref_in_actor=False (not using LoRA mode where ref is actor without adapter)
                        #   - ema_tau < 1.0 (EMA is actually enabled; tau=1.0 means no update)
                        #   - global_steps > 0 (skip the first step)
                        #   - global_steps is a multiple of ema_update_period
                        ...
                        print(f"[DEBUG EMA] global_steps={self.global_steps}, ema_tau={ema_tau}, ema_update_period={ema_update_period}, "
                              f"use_reference_policy={self.use_reference_policy}, ref_in_actor={self.ref_in_actor}")
                        if (
                            ...
                            print(f"[DEBUG EMA] Updating reference policy with EMA at step {self.global_steps}, tau={ema_tau}")
                            with marked_timer("update_ref_policy_ema", timing_raw):
                                self.ref_policy_wg.update_ref_policy_ema(tau=ema_tau)
                            print(f"[DEBUG EMA] Reference policy EMA update completed")
                            metrics["ref_policy/ema_tau"] = ema_tau
                            metrics["ref_policy/ema_update_step"] = self.global_steps
                    ...
                    # validate reference policy (disabled by default, set eval_ref_policy_freq > 0 to enable)
                    eval_ref_policy_freq = self.config.trainer.get("eval_ref_policy_freq", 0)
                    if (
                        ...
                        with marked_timer("testing_ref_policy", timing_raw):
                            ref_val_metrics: dict = self._validate_ref_policy_agent()
                        metrics.update(ref_val_metrics)

                    if self.config.trainer.save_freq > 0 and self.global_steps % self.config.trainer.save_freq == 0:
                        with marked_timer("save_checkpoint", timing_raw):
                            self._save_checkpoint()

                # collect metrics (data_metrics computed before masking truncated samples)
                ...
                if self.global_steps >= self.total_training_steps:
                    ...
                    if self.val_reward_fn is not None:
                        ...
                        # Also validate reference policy at end if enabled
                        eval_ref_policy_freq = self.config.trainer.get("eval_ref_policy_freq", 0)
                        if eval_ref_policy_freq > 0 and self.use_reference_policy and not self.ref_in_actor:
                            ref_val_metrics = self._validate_ref_policy_agent()
                            pprint(f"Final reference policy validation metrics: {ref_val_metrics}")
                            logger.log(data=ref_val_metrics, step=self.global_steps)
                    return

    def _validate_agent(self, save_output_path: str | None = None):
        """
        Run validation with optional saving of results.

        Args:
            save_output_path: If provided, save raw results to this path for pass@k evaluation.
                             Uses _validate_agent_and_save instead of _validate_agent_inner.

        Returns:
            Dictionary of validation metrics
        """
        # Temporarily override max_response_length if val_kwargs specifies it
        val_max_response_length = getattr(self.config.actor_rollout_ref.rollout.val_kwargs, "max_response_length", None)
        original_max_response_length = None
        if val_max_response_length is not None:
            original_max_response_length = self.agent_execution_engine.max_response_length
            self.agent_execution_engine.max_response_length = val_max_response_length
            print(f"Using validation max_response_length: {val_max_response_length} (training: {original_max_response_length})")

        try:
            if save_output_path is not None:
                return self._validate_agent_and_save(output_path=save_output_path)
            else:
                return self._validate_agent_inner()
        finally:
            # Restore original max_response_length if it was overridden
            if original_max_response_length is not None:
                self.agent_execution_engine.max_response_length = original_max_response_length

    def _validate_agent_inner(self):
        rewards_lst = []
        data_source_lst = []
        uid_lst = []
    ...
    def _validate_agent_and_save(self, output_path: str):
        """
        Run validation and save raw results to parquet for pass@k evaluation.

        This method is similar to _validate_agent_inner() but also collects and saves
        the raw data needed for pass@k evaluation:
        - data_source: Dataset name (AIME, AMC, MATH, etc.)
        - problem: The problem text
        - ground_truth: Expected answer
        - responses: List of K responses
        - rewards: List of K reward scores

        Args:
            output_path: Path to save the parquet file with results

        Returns:
            Dictionary of validation metrics (same as _validate_agent_inner)
        """
        ...
        for test_data in self.val_dataloader:
            ...
            # Collect extra info for saving
            extra_info = test_batch.non_tensor_batch.get("extra_info", [{}] * reward_tensor.shape[0])
            for info in extra_info:
                if isinstance(info, str):
                    info = json.loads(info)
                problems_lst.append(info.get("question", info.get("problem", "")))
                ground_truths_lst.append(info.get("ground_truth", info.get("answer", "")))

            # Decode responses from tokens
            response_tokens = test_batch.batch["responses"]
            for i in range(response_tokens.shape[0]):
                response = self.tokenizer.decode(response_tokens[i], skip_special_tokens=True)
                responses_lst.append(response)

        # Handle empty validation set
        if not rewards_lst:
            print("Warning: No validation data found. Skipping save.")
            return {}
        ...
        # Group by uid to create pass@k compatible format
        uid_to_data = {}
        for i in range(reward_tensor.shape[0]):
            uid = uid_tensor[i]
            data_source = data_sources[i]
            ...
            if uid not in uid_to_data:
                uid_to_data[uid] = {
                    ...
                    "problem": problems_lst[i] if i < len(problems_lst) else "",
                    "ground_truth": ground_truths_lst[i] if i < len(ground_truths_lst) else "",
                    "responses": [],
                    "rewards": [],
                }
            uid_to_data[uid]["responses"].append(responses_lst[i] if i < len(responses_lst) else "")
            uid_to_data[uid]["rewards"].append(reward)

        # Convert to list for DataFrame
        results_data = list(uid_to_data.values())
        ...
        for ds in df["data_source"].unique():
            n_ds = len(df[df["data_source"] == ds])
            print(f"  {ds}: {n_ds} problems")

        # Also compute and return standard metrics
        data_source_reward = {}
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            data_source_uid_pass_rates[data_source][uid] = max(
                data_source_uid_pass_rates[data_source][uid],
    ...
    def _validate_ref_policy_agent(self):
        """Validate using the reference policy instead of the actor policy.

        This method sets a flag that causes rollout_mode() to use reference policy
        weights instead of actor weights for generation. After validation completes,
        the flag is cleared so subsequent generations use actor weights.

        Returns:
            Dictionary of validation metrics with 'ref_' prefix.
        """
        if not self.use_reference_policy or self.ref_in_actor:
            print("Warning: _validate_ref_policy_agent called but reference policy not available or using LoRA mode")
            return {}

        print("Starting reference policy validation...")

        # Set flag to use reference policy weights for generation
        # rollout_mode() will check this flag and override with ref weights
        self.actor_rollout_wg.set_use_ref_weights_for_generation(use_ref_weights=True)

        try:
            # Run validation with reference policy weights
            # _validate_agent() -> trajectory_generator() -> wake_up() -> rollout_mode()
            # rollout_mode() will see the flag and use ref weights
            ref_metrics = self._validate_agent()

            # Prefix all metrics with 'ref_' to distinguish from actor metrics
            ref_metrics = {f"ref_{k}": v for k, v in ref_metrics.items()}

        finally:
            # Always clear the flag, even if validation fails
            # This ensures next generation uses actor weights
            self.actor_rollout_wg.set_use_ref_weights_for_generation(use_ref_weights=False)

        print("Reference policy validation complete")
        return ref_metrics
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict]):
        ...
        for traj in trajectories:
            ...
            all_rollout_log_probs_list.append(traj["rollout_log_probs"])
        ...
        # rollout log probs (right pad with 0.0, same shape as response_batch)
