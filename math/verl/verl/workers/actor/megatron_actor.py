...
from verl.utils.megatron.tensor_parallel import vocab_parallel_full_forward_kl, vocab_parallel_full_reverse_kl
...
class MegatronPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False, return_logits=False) -> torch.Tensor:
        ...
        Args:
            ...
            return_logits: Whether to return full logits (for full KL computation).

        Returns:
            If return_logits=False (default):
                log_probs: tensor of shape [batch_size, response_length]
                entropys: tensor of shape [batch_size, response_length] or empty tensor
            If return_logits=True:
                log_probs: tensor of shape [batch_size, response_length]
                entropys: tensor of shape [batch_size, response_length] or empty tensor
                logits: tensor of shape [batch_size, response_length, vocab_size // tp_size] (TP-sharded)
        """
        if return_logits and self.use_fused_kernels:
            raise ValueError(
                "return_logits=True is not compatible with use_fused_kernels=True. "
        ...
        def compute_logprobs_fn(output, data, use_dynamic_bsz=False, indices=None):
            ...
            # Also return logits if available (for full KL computation)
            if "logits" in output:
                result["logits"] = output["logits"][:, -response_length - 1 : -1, :].contiguous()
            return result
        ...
        if recompute_old_log_prob:
            ...
            with torch.no_grad():
                ...
                if return_logits:
                    # Extract logits from output (only on last PP stage, but sharded across TP)
                    if mpu.is_pipeline_last_stage(ignore_virtual=True):
                        if calculate_entropy:
                            logits_list = [o[0]["logits"] for o in output["output"]]
                        else:
                            logits_list = [o["logits"] for o in output["output"]]
                        output_logits = torch.cat(logits_list, dim=0).to(torch.float32)
                        if use_dynamic_bsz:
                            indices = output["indices"]
                            indices = list(itertools.chain.from_iterable(indices))
                            revert_indices = torch.tensor(get_reverse_idx(indices), dtype=torch.long)
                            output_logits = output_logits[revert_indices]
                        vocab_size_per_tp = output_logits.size(-1)
                    else:
                        # Get vocab_size_per_tp from TP rank 0 on last PP stage via broadcast
                        vocab_size_per_tp = torch.tensor([0], dtype=torch.long, device=get_device_id())
                    # Broadcast vocab_size_per_tp from last PP stage
                    vocab_size_tensor = torch.tensor([vocab_size_per_tp if mpu.is_pipeline_last_stage(ignore_virtual=True) else 0],
                                                     dtype=torch.long, device=get_device_id())
                    torch.distributed.broadcast(
                        tensor=vocab_size_tensor,
                        src=mpu.get_pipeline_model_parallel_last_rank(),
                        group=mpu.get_pipeline_model_parallel_group(),
                        async_op=False,
                    )
                    vocab_size_per_tp = vocab_size_tensor.item()
                    if not mpu.is_pipeline_last_stage(ignore_virtual=True):
                        output_logits = torch.empty(
                            size=(batch_size, response_length, vocab_size_per_tp),
                            dtype=torch.float32, device=get_device_id()
                        )
                    else:
                        output_logits = output_logits.to(get_device_id())
                    # Broadcast logits across PP ranks
                    torch.distributed.broadcast(
        ...
        if return_logits:
            return log_probs, entropys, output_logits
        else:
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        if self.config.use_kl_loss:
            ...
            if kl_loss_type in ("full_forward", "full_reverse"):
                # Full KL requires ref_logits (TP-sharded) instead of ref_log_prob
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if forward_only:
                ...
                if self.config.use_kl_loss:
                    ...
                    if kl_loss_type in ("full_forward", "full_reverse"):
                        # Check for unsupported top-k with Megatron
                        if kl_topk is not None:
                            raise NotImplementedError(
                                "kl_topk_tokens is not yet supported for Megatron backend with tensor parallelism. "
                                "Top-k KL requires distributed top-k across TP ranks which is not implemented. "
                                "Please use kl_topk_tokens=null or switch to FSDP backend."
                            )
                        # Full KL requires logits from both current policy and reference policy
                        # For Megatron with TP, logits are sharded across TP ranks
                        actor_logits = output["logits"][:, -response_length - 1 : -1, :].contiguous()
                        ref_logits = data["ref_logits"]
                        if kl_loss_type == "full_forward":
                            kld = vocab_parallel_full_forward_kl(actor_logits, ref_logits)
                        else:
                            kld = vocab_parallel_full_reverse_kl(actor_logits, ref_logits)
                    else:
        ...
        def forward_step(batch_iter, model, return_schedule_plan: bool = False):
            ...
            if self.use_fused_kernels:
                ...
                # Check if we need to keep logits for full KL computation
                need_logits_for_kl = (not forward_only) and self.config.use_kl_loss and self.config.kl_loss_type in ("full_forward", "full_reverse")
                ...
                def logits_processor(logits, label, label_mask):
                    ...
                    ret = {}
                    # Keep logits if needed for return or full KL computation
                    if return_logits or need_logits_for_kl or calculate_entropy:
                        logits_bak = logits.clone()
                        if calculate_entropy or return_logits or need_logits_for_kl:
                            logger.warning_once(
                    ...
                    # Return logits if requested (for full KL computation)
                    if return_logits or need_logits_for_kl:
                        ret["logits"] = logits_bak
                    return ret
