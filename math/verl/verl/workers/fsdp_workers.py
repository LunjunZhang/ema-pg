...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        # Use tokenizer_path if specified (useful when model_path points to a checkpoint without tokenizer files)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # Flag to use reference policy weights instead of actor weights for generation
        # Set via set_use_ref_weights_for_generation() before validation
        self._use_ref_weights_for_generation: bool = False
    ...
    def set_use_ref_weights_for_generation(self, use_ref_weights: bool):
        ...
        When True, rollout_mode() will sync reference policy weights to vLLM
        instead of actor weights, enabling generation with the reference policy.

        Args:
            use_ref_weights: If True, use reference policy for next generation.
    ...
    async def rollout_mode(self):
        ...
        # If using reference policy for generation, override with ref weights
        # This is used for reference policy evaluation
        if self._use_ref_weights_for_generation:
            await self._override_with_ref_weights()
            log_gpu_memory_usage("After override_with_ref_weights", logger=logger)
    ...
    async def _override_with_ref_weights(self):
        """Override vLLM weights with reference policy weights.

        Called during rollout_mode() when _use_ref_weights_for_generation is True.
        This allows generating using reference policy instead of actor policy.
        """
        # Get the reference worker from fused_worker_dict
        if not hasattr(self, "fused_worker_dict") or self.fused_worker_dict is None:
            raise RuntimeError("fused_worker_dict not available. Reference policy must be colocated with actor.")

        ref_worker = self.fused_worker_dict.get("ref", None)
        if ref_worker is None:
            raise RuntimeError(f"Reference worker not found in fused_worker_dict. Available keys: {list(self.fused_worker_dict.keys())}")

        if not hasattr(ref_worker, "ref_module_fsdp") or ref_worker.ref_module_fsdp is None:
            raise RuntimeError("Reference policy model not found in reference worker.")

        # Load reference model to GPU if offloaded
        if ref_worker._is_offload_param:
            load_fsdp_model_to_gpu(ref_worker.ref_module_fsdp)

        # Collect weights from reference model
        params = ref_worker.ref_module_fsdp.state_dict()
        params = convert_weight_keys(
            params, getattr(ref_worker.ref_module_fsdp, "_fsdp_wrapped_module", ref_worker.ref_module_fsdp)
        )
        ...
        # Update vLLM with reference weights (overriding the actor weights just synced)
        await self.rollout.update_weights(per_tensor_param, peft_config=None, base_sync_done=False)

        del params, per_tensor_param
        aggressive_empty_cache(force_sync=True)

        if self.rank == 0:
            print("Override with reference policy weights for generation")
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            # Create a checkpoint manager for the reference policy (model only, no optimizer)
            ref_checkpoint_contents = OmegaConf.create({"load_contents": ["model"], "save_contents": ["model"]})
            self.ref_checkpoint_manager = FSDPCheckpointManager(
                model=self.ref_module_fsdp,
                optimizer=None,
                lr_scheduler=None,
                processing_class=self.processor if self.processor is not None else self.tokenizer,
                checkpoint_config=ref_checkpoint_contents,
            )
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="compute_log_prob_with_logits")
    def compute_log_prob_with_logits(self, data: DataProto):
        """Compute log probability and extract logits for KL divergence.

        This unified function handles both:
        - Full logits (kl_topk_k=-1): returns full vocab logits
        - Top-k logits (kl_topk_k>0): returns top-k logits and indices

        Expects data.meta_info["kl_topk_k"] to be set by caller.
        """
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.ulysses_sharding_manager:
            ...
            # - entropys: used for entropy logging in ray_trainer.py
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_log_prob_with_logits", logger=logger)
    ...
    def compute_ref_log_prob(self, data: DataProto):
        """Compute reference policy log probabilities (original function, backward compatible)."""
        if self._is_lora:
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="olive", role="ref_compute_log_prob_at_indices")
    def compute_ref_log_prob_at_indices(self, data: DataProto):
        ...
        This is used for reverse KL where we gather ref logits at actor's top-k indices.

        Expects data.batch["kl_topk_indices"] to be set by caller (actor's top-k indices).

        Returns:
            DataProto containing:
                - ref_log_prob: tensor of shape [batch_size, response_length]
                - ref_logits_k: tensor of shape [batch_size, response_length, k]
                - ref_logsumexp: tensor of shape [batch_size, response_length]
                - ref_topk_indices: tensor of shape [batch_size, response_length, k]
        """
        if self._is_lora:
            raise ValueError(
                "compute_ref_log_prob_at_indices is not supported in LoRA mode (ref_in_actor=True). "
                "Full KL divergence requires a separate reference model."
            )
        assert self._is_ref
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(tensors={
                ...
                "ref_topk_indices": kl_inputs["topk_indices"],
            })
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="olive", role="ref_compute_log_prob_with_logits")
    def compute_ref_log_prob_with_logits(self, data: DataProto):
        """Compute ref log probability and extract ref's logits for KL divergence.
        ...
        Returns:
            DataProto containing:
                - ref_log_prob: tensor of shape [batch_size, response_length]
                - ref_logits_k: tensor of shape [batch_size, response_length, vocab_size or k]
                - ref_logsumexp: tensor of shape [batch_size, response_length]
                - ref_topk_indices: tensor of shape [batch_size, response_length, k] (only if kl_topk_k>0)
        """
        if self._is_lora:
            raise ValueError(
                "compute_ref_log_prob_with_logits is not supported in LoRA mode (ref_in_actor=True). "
                "Full KL divergence requires a separate reference model."
            )
        assert self._is_ref
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    @DistProfiler.annotate(color="magenta", role="update_ref_policy_ema")
    def update_ref_policy_ema(self, tau: float):
        ...
        Note:
            ...
            - For LoRA mode (ref_in_actor=True), this method should not be called as the
              reference is the actor without LoRA applied.
            - For FSDP1, uses summon_full_params sequentially to avoid OOM (actor params copied
              to CPU, then ref params updated).
        ...
        # Debug: print contents of fused_worker_dict
        print(f"[DEBUG EMA] fused_worker_dict keys: {list(self.fused_worker_dict.keys())}")
        print(f"[DEBUG EMA] fused_worker_dict contents: {self.fused_worker_dict}")
        ...
        # Check if this reference worker has a separate model (not LoRA mode)
        ...
        if actor_worker._is_offload_param:
            load_fsdp_model_to_gpu(actor_worker.actor_module_fsdp)
        ...
        fsdp_ver = fsdp_version(ref_model)
        print(f"[DEBUG EMA] Using FSDP version {fsdp_ver}")
        ...
        if actor_worker._is_offload_param:
            offload_fsdp_model_to_cpu(actor_worker.actor_module_fsdp)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_ref_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        """Save the reference policy checkpoint (model weights only, no optimizer).
        ...
        if not hasattr(self, "ref_checkpoint_manager") or self.ref_checkpoint_manager is None:
            raise RuntimeError(
                "ref_checkpoint_manager not found. Make sure the reference policy worker "
                "is properly initialized with a checkpoint manager."
            )

        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.ref_module_fsdp)

        self.ref_checkpoint_manager.save_checkpoint(
            local_path=local_path, hdfs_path=hdfs_path, global_step=global_step, max_ckpt_to_keep=max_ckpt_to_keep
        )
        dist.barrier()
        ...
        if self.rank == 0:
            print(f"Saved reference policy checkpoint to {local_path}")

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_ref_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        """Load the reference policy checkpoint (model weights only).
        ...
        # No checkpoint to load, just offload the model to CPU if needed
        if local_path is None:
            if self._is_offload_param:
                offload_fsdp_model_to_cpu(self.ref_module_fsdp)
            return
        ...
        self.ref_checkpoint_manager.load_checkpoint(
            local_path=local_path, hdfs_path=hdfs_path, del_local_after_load=del_local_after_load
        )
