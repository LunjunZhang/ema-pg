...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def compute_ref_log_prob(self, data: DataProto, return_logits: bool = False):
        assert self._is_ref
        if self._ref_is_offload_param:
        ...
        if return_logits:
            # Full KL requires logits - check fused kernels compatibility
            if self.config.ref.get("use_fused_kernels", False):
                raise ValueError(
                    "return_logits=True is not compatible with use_fused_kernels=True. "
                    "Fused kernels compute log_probs directly without materializing logits. "
                    "To use full KL divergence, set use_fused_kernels=False."
                )
            ref_log_prob, _, ref_logits = self.ref_policy.compute_log_prob(
                data=data, calculate_entropy=False, return_logits=True
            )
            output = DataProto.from_dict(tensors={"ref_log_prob": ref_log_prob, "ref_logits": ref_logits})
        else:
            ref_log_prob, _ = self.ref_policy.compute_log_prob(data=data, calculate_entropy=False)
    ...
    def update_ref_policy_ema(self, tau: float):
        ...
        if actor_worker is None:
            raise RuntimeError(
                "Actor worker not found in fused_worker_dict. "
                "Make sure the actor is enabled and colocated with the reference policy."
            )

        # Check if actor worker has the model
        if not hasattr(actor_worker, "actor_module") or actor_worker.actor_module is None:
        ...
        if not hasattr(self, "ref_module") or self.ref_module is None:
            raise RuntimeError(
        ...
        if actor_worker._is_offload_param:
            load_megatron_model_to_gpu(actor_worker.actor_module, load_grad=False)
            log_gpu_memory_usage("After load actor params during update_ref_policy_ema", logger=logger)

        if self._ref_is_offload_param:
            load_megatron_model_to_gpu(self.ref_module, load_grad=False)
            log_gpu_memory_usage("After load ref params during update_ref_policy_ema", logger=logger)

        # Megatron models are lists of model chunks (for pipeline parallelism)
        # Both actor and ref should have the same number of chunks
        if len(actor_worker.actor_module) != len(self.ref_module):
            raise RuntimeError(
                f"Model chunk count mismatch: actor has {len(actor_worker.actor_module)}, "
                f"ref has {len(self.ref_module)}"
            )

        with torch.no_grad():
            for actor_model, ref_model in zip(actor_worker.actor_module, self.ref_module, strict=True):
                ...
                for name, ref_param in ref_param_dict.items():
                    ...
                    ref_param.data.mul_(tau).add_(actor_param.data, alpha=1.0 - tau)

                for name, ref_buf in ref_buffer_dict.items():
                    actor_buf = actor_buffer_dict[name]
                    ref_buf.data.mul_(tau).add_(actor_buf.data, alpha=1.0 - tau)
        ...
        if actor_worker._is_offload_param:
            offload_megatron_model_to_cpu(actor_worker.actor_module)
            log_gpu_memory_usage("After offload actor params during update_ref_policy_ema", logger=logger)

        if self._ref_is_offload_param:
            offload_megatron_model_to_cpu(self.ref_module)
            log_gpu_memory_usage("After offload ref params during update_ref_policy_ema", logger=logger)
