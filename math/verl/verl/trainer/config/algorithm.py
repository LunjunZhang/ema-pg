...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        norm_adv_by_batch_std (bool): Whether to normalize advantages by batch-wise std.
            Must be False if norm_adv_by_std_in_grpo is True.
        use_kl_in_reward (bool): Whether to enable in-reward KL penalty.
        kl_penalty (str): How to estimate KL divergence: "kl", "abs", "mse", "low_var_kl", or "full".
        kl_penalty_at_final_token (bool): If True, sum up KL for all tokens and apply at the final token only,
            making KL penalty sequence-level like the reward. Only effective when use_kl_in_reward=True.
            Note: This is non-differentiable (used in reward shaping, not gradient computation).
            Default is False (token-level KL penalty).
        ...
        ref_policy_ema_tau (float): EMA coefficient for reference policy update.
            When less than 1.0, the reference policy is updated via:
            ref_policy = ema_tau * ref_policy + (1 - ema_tau) * current_policy.
            Default is 1.0 (no update, original behavior).
        ref_policy_ema_update_period (int): How often (in gradient steps) to update the reference policy.
            Default is 10. Only has effect when ref_policy_ema_tau < 1.0.
    """
    ...
    kl_penalty: str = "kl"
    kl_penalty_at_final_token: bool = False
    ...
    # Reference policy EMA update: updates reference policy towards current policy
    # When ref_policy_ema_tau=1.0 (default), reference policy stays fixed (original behavior)
    ref_policy_ema_tau: float = 1.0
    ref_policy_ema_update_period: int = 10
