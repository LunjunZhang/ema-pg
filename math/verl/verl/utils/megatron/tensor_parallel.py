...
def vocab_parallel_full_forward_kl(vocab_parallel_logits: torch.Tensor, vocab_parallel_ref_logits: torch.Tensor) -> torch.Tensor:
    """Compute full forward KL divergence KL(π_ref || π) when logits are sharded across TP ranks.

    Args:
        vocab_parallel_logits: Logits from current policy, shape (batch, seq, vocab_size // tp_size)
        vocab_parallel_ref_logits: Logits from reference policy, shape (batch, seq, vocab_size // tp_size)

    Returns:
        kl: Forward KL divergence per token, shape (batch, seq)
    """
    # Compute ref_probs = softmax(ref_logits) with distributed normalization
    ref_logits_max = vocab_parallel_ref_logits.max(dim=-1, keepdim=True).values
    dist.all_reduce(ref_logits_max, op=dist.ReduceOp.MAX, group=mpu.get_tensor_model_parallel_group())
    normalized_ref_logits = vocab_parallel_ref_logits - ref_logits_max
    ref_exp = normalized_ref_logits.exp()
    ref_sum_exp = ref_exp.sum(dim=-1, keepdim=True)
    dist.all_reduce(ref_sum_exp, group=mpu.get_tensor_model_parallel_group())
    ref_probs = ref_exp / ref_sum_exp  # softmax(ref_logits)
    ref_log_probs = normalized_ref_logits - ref_sum_exp.log()  # log_softmax(ref_logits)

    # Compute log_probs = log_softmax(logits) with distributed normalization
    logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
    dist.all_reduce(logits_max, op=dist.ReduceOp.MAX, group=mpu.get_tensor_model_parallel_group())
    normalized_logits = vocab_parallel_logits - logits_max
    sum_exp = normalized_logits.exp().sum(dim=-1, keepdim=True)
    dist.all_reduce(sum_exp, group=mpu.get_tensor_model_parallel_group())
    log_probs = normalized_logits - sum_exp.log()  # log_softmax(logits)

    # Forward KL = Σ_v ref_probs(v) * (ref_log_probs(v) - log_probs(v))
    kl_local = (ref_probs * (ref_log_probs - log_probs)).sum(dim=-1)
    dist.all_reduce(kl_local, group=mpu.get_tensor_model_parallel_group())
    return kl_local


def vocab_parallel_full_reverse_kl(vocab_parallel_logits: torch.Tensor, vocab_parallel_ref_logits: torch.Tensor) -> torch.Tensor:
    """Compute full reverse KL divergence KL(π || π_ref) when logits are sharded across TP ranks.
    ...
    Returns:
        kl: Reverse KL divergence per token, shape (batch, seq)
    """
    # Compute probs = softmax(logits) with distributed normalization
    logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
    dist.all_reduce(logits_max, op=dist.ReduceOp.MAX, group=mpu.get_tensor_model_parallel_group())
    normalized_logits = vocab_parallel_logits - logits_max
    ...
    dist.all_reduce(sum_exp, group=mpu.get_tensor_model_parallel_group())
    probs = exp_logits / sum_exp  # softmax(logits)
    log_probs = normalized_logits - sum_exp.log()  # log_softmax(logits)

    # Compute ref_log_probs = log_softmax(ref_logits) with distributed normalization
    ref_logits_max = vocab_parallel_ref_logits.max(dim=-1, keepdim=True).values
    dist.all_reduce(ref_logits_max, op=dist.ReduceOp.MAX, group=mpu.get_tensor_model_parallel_group())
    normalized_ref_logits = vocab_parallel_ref_logits - ref_logits_max
    ref_sum_exp = normalized_ref_logits.exp().sum(dim=-1, keepdim=True)
    dist.all_reduce(ref_sum_exp, group=mpu.get_tensor_model_parallel_group())
    ref_log_probs = normalized_ref_logits - ref_sum_exp.log()  # log_softmax(ref_logits)

    # Reverse KL = Σ_v probs(v) * (log_probs(v) - ref_log_probs(v))
    kl_local = (probs * (log_probs - ref_log_probs)).sum(dim=-1)
    dist.all_reduce(kl_local, group=mpu.get_tensor_model_parallel_group())
    return kl_local
